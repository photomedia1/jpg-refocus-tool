...
CxImageMNG::~CxImageMNG()
{
	...
	// free objects
	if (mnginfo.image) free_mem(mnginfo.image);
	if (mnginfo.alpha) free_mem(mnginfo.alpha);
...
bool CxImageMNG::Encode(CxFile *hFile)
{
	...
	cx_try
	{
		...
		mnginfo.image = (uint8_t*)alloc_mem(head.biSizeImage);
