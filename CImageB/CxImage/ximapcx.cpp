...
bool CxImagePCX::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
    ...
    lpHead1 = pcximage = (uint8_t*)alloc_mem(nbytes);
    while (nbytes > 0){
    ...
    lpHead2 = pcxpixels = (uint8_t *)alloc_mem(Width + pcxHeader.BytesPerLine * 8);
    // Convert the image
    for (y = 0; y < Height; y++){
	...
	if (lpHead1){ free_mem(lpHead1); lpHead1 = NULL; }
    if (lpHead2){ free_mem(lpHead2); lpHead2 = NULL; }
	return false;
  }
	if (lpHead1){ free_mem(lpHead1); lpHead1 = NULL; }
    if (lpHead2){ free_mem(lpHead2); lpHead2 = NULL; }
  ...
  cx_try
  {
	...
	if (head.biClrUsed==0){
		...
		if (head.biBitCount == 8){
			hFile->PutC(0x0C);
			uint8_t* pal = (uint8_t*)alloc_mem(768);
			RGBQUAD c;
		...
		uint8_t* plane = (uint8_t*)alloc_mem(pcxHeader.BytesPerLine);
		uint8_t* raw = (uint8_t*)alloc_mem(head.biWidth);
