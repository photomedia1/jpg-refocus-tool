  ...
  cx_try
  {
	...
	if (isRGB) {
		...
		if (tiled_image){
			...
			tilebuf = (uint8_t*)alloc_mem(TIFFTileSize(m_tif));
		}
		
		bits = (uint8_t*)alloc_mem(bitspersample==16? bitsize*2 : bitsize); // * VK
		...
		if (!tiled_image && bitspersample==16) {					  // + VK +
			...
			bits16 = bits;
			bits   = (uint8_t*)alloc_mem(bitsize);
		}

		if (bits==NULL){
			if (bits16) free_mem(bits16);								  // + VK
			if (pal)	free_mem(pal);									  // + VK
			if (tilebuf)free_mem(tilebuf);								  // + VK	
		...
		if (bits16) row_shifts = (uint8_t*)alloc_mem(height); 
#endif
		...
		if (row_shifts)	free_mem( row_shifts );
#endif

		if (tiled_image) free_mem(tilebuf);
		if (pal)		 free_mem(pal);
...
//	TIFFSetField(m_tif, TIFFTAG_YPOSITION, (float)info.yOffset);
	...
	switch(bitcount) {				
		...
		{
			if (samplesperpixel==1){
				bits = (uint8_t*)alloc_mem(info.dwEffWidth);
				...
				bits = (uint8_t*)alloc_mem(2*width);
				if (!bits) return false;
				for (y = 0; y < height; y++) {
					for (x=0;x<width;x++){
		...
		case 24:
		{
			uint8_t *buffer = (uint8_t *)alloc_mem(info.dwEffWidth);
		...
		{
#if CXIMAGE_SUPPORT_ALPHA
			uint8_t *buffer = (uint8_t *)alloc_mem((info.dwEffWidth*4)/3);
			if (!buffer) return false;
			for (y = 0; y < height; y++) {
				// get a pointer to the scanline
				memcpy(buffer, info.pImage + (height - y - 1)*info.dwEffWidth, info.dwEffWidth);
