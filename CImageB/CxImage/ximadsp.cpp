...
bool CxImage::Median(int32_t Ksize)
{
	...
	RGBQUAD* kernel = (RGBQUAD*)alloc_mem(Ksize*Ksize*sizeof(RGBQUAD));
				   ...
				   int32_t direction, bool bForceFFT, bool bMagnitude)
{
	...
	grid = (_complex **)alloc_mem(w * sizeof(_complex));
	for (k=0;k<w;k++) {
		grid[k] = (_complex *)alloc_mem(h * sizeof(_complex));
	}
	for (j=0;j<h;j++) {
		for (k=0;k<w;k++) {
	...
	//DFT buffers
	double *real2,*imag2;
	real2 = (double*)alloc_mem(max(w,h) * sizeof(double));
	imag2 = (double*)alloc_mem(max(w,h) * sizeof(double));

	/* Transform the rows */
	real = (double *)alloc_mem(w * sizeof(double));
	imag = (double *)alloc_mem(w * sizeof(double));
	...
	real = (double *)alloc_mem(h * sizeof(double));
	imag = (double *)alloc_mem(h * sizeof(double));
...
bool CxImage::GaussianBlur(float radius /*= 1.0f*/, CxImage* iDst /*= 0*/)
{
	...
	// blur the cols
    uint8_t* cur_col = (uint8_t*)alloc_mem(bypp*head.biHeight);
    uint8_t* dest_col = (uint8_t*)alloc_mem(bypp*head.biHeight);
