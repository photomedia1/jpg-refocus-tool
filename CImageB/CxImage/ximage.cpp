...
bool CxImage::Destroy()
{
	...
	if (info.pGhost==NULL){
		...
		if (pSelection) {free_mem(pSelection); pSelection=0;}
		if (pAlpha) {free_mem(pAlpha); pAlpha=0;}
		if (pDib) {free_mem(pDib); pDib=0;}
		return true;
	}
	return false;
}
...
void CxImage::Copy(const CxImage &src, bool copypixels, bool copyselection, bool copyalpha)
{
	...
	if (copyselection && src.pSelection){
		if (pSelection) free_mem(pSelection);
		pSelection = (uint8_t*)alloc_mem(nSize);
		memcpy(pSelection,src.pSelection,nSize);
	}
	//copy the alpha channel
	if (copyalpha && src.pAlpha){
		if (pAlpha) free_mem(pAlpha);
		pAlpha = (uint8_t*)alloc_mem(nSize);
...
//    head.biClrImportant = 0;  See SetClrImportant
	...
	pDib = alloc_mem(GetSize()); // alloc memory block to store our bitmap
    if (!pDib){
