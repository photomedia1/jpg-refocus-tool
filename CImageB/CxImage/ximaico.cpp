...
bool CxImageICO::Decode(CxFile *hFile)
{
	...
	if ((icon_header.idReserved == 0) && ((icon_header.idType == 1)||(icon_header.idType == 2))) {
		...
		ICONDIRENTRY *icon_list = (ICONDIRENTRY *)alloc_mem(icon_header.idCount * sizeof(ICONDIRENTRY));
		int32_t c;
		...
		if ((page>=0)&&(page<icon_header.idCount)){
			...
			{	// standard icon
				...
				if (c<=24){
					...
					uint8_t* buf=(uint8_t*)alloc_mem(4*head.biHeight*head.biWidth);
					uint8_t* src = buf;
				...
				uint8_t *mask = (uint8_t *)alloc_mem(masksize);
...
#if CXIMAGE_SUPPORT_ALPHA
			if (AlphaIsValid() && head.biClrUsed==0){
				uint8_t* buf=(uint8_t*)alloc_mem(imagesize);
				uint8_t* dst = buf;
				for (int32_t y = 0; y < head.biHeight; y++) {
