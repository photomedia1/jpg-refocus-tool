...
TIFFInitOJPEG(TIFF* tif, int scheme)
{
	...
	/* state block */
	sp=(OJPEGState*)_TIFFmalloc(sizeof(OJPEGState));
	if (sp==NULL)
	{
...
OJPEGWriteHeaderInfo(TIFF* tif)
{
	...
	if ((sp->subsampling_force_desubsampling_inside_decompression==0) && (sp->samples_per_pixel_per_plane>1))
	{
		...
		if (sp->subsampling_convert_log==0)
		{
			...
			sp->subsampling_convert_ycbcrbuf=(uint8*)_TIFFmalloc(sp->subsampling_convert_ycbcrbuflen);
			if (sp->subsampling_convert_ycbcrbuf==0)
			{
				TIFFErrorExt(tif->tif_clientdata,module,"Out of memory");
				return(0);
			...
			sp->subsampling_convert_ycbcrimage=(uint8**)_TIFFmalloc(sp->subsampling_convert_ycbcrimagelen*sizeof(uint8*));
			if (sp->subsampling_convert_ycbcrimage==0)
			{
				TIFFErrorExt(tif->tif_clientdata,module,"Out of memory");
				return(0);
...
OJPEGWriteStreamSoi(TIFF* tif, void** mem, uint32* len)
{
	...
	*mem=(void*)sp->out_buffer;
	sp->out_state = (OJPEGStateOutState)((int)sp->out_state + 1);
}
...
{
	return(SETJMP(sp->exit_jmpbuf)?0:(jpeg_read_scanlines(cinfo,(JSAMPARRAY)scanlines,max_lines),1));
}
#endif
...
{
	return(SETJMP(sp->exit_jmpbuf)?0:(jpeg_read_raw_data(cinfo,(JSAMPIMAGE)data,max_lines),1));
}
#endif
