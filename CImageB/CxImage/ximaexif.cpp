...
{
	for(int32_t i=0;i<MAX_SECTIONS;i++) if(Sections[i].Data) free_mem(Sections[i].Data);
...
bool CxImageJPG::CxExifInfo::DecodeExif(CxFile * hFile, int32_t nReadMode)
{
    ...
    for(;;){
        ...
        Data = (uint8_t *)alloc_mem(itemlen);
        if (Data == NULL){
        ...
        switch(marker){
            ...
            case M_COM: // Comment section
                if (HaveCom || ((nReadMode & EXIF_READ_EXIF) == 0)){
                    ...
                    free_mem(Sections[--SectionsRead].Data);
