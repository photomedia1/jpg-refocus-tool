...
bool CxMemFile::Alloc(uint32_t dwNewLen)
{
	if (dwNewLen > (uint32_t)m_Edge)
	{
		...
		// allocate new buffer
		if (m_pBuffer == NULL) m_pBuffer = (uint8_t*)alloc_mem(dwNewBufferSize);
		else	m_pBuffer = (uint8_t*)realloc_mem(m_pBuffer, dwNewBufferSize);
