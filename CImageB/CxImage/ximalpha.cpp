...
{
	if (pAlpha==NULL) {
		pAlpha = (uint8_t*)alloc_mem(head.biWidth * head.biHeight);
...
bool CxImage::AlphaCopy(CxImage &from)
{
	...
	if (pAlpha==NULL) pAlpha = (uint8_t*)alloc_mem(head.biWidth * head.biHeight);
...
bool CxImage::AlphaFlip()
{
	...
	uint8_t *buff = (uint8_t*)alloc_mem(head.biWidth);
...
{
	if (!pAlpha) return false;
	uint8_t* pAlpha2 = (uint8_t*)alloc_mem(head.biWidth * head.biHeight);
